...
def _get_gitclone_links_with_reponame( username, page = 1 ):
    ''' returns all the clone links in list '''
    response = requests.get( ''.join([ 
            USER_API_URL, 
            '{}/repos?per_page=100&page={}'.format( 
                username, 
                page 
            )
        ])
...
def main():
    ...
    if user_info:
        ...
        for counter in range( number_of_pages ):
            name_and_links = _get_gitclone_links_with_reponame( user, counter + 1 )
            if number_of_repos > 0:
                for repo in name_and_links:
                    # print("({}/{})  {}".format( count, number_of_repos, repo.name ))
