...
def get_clone_urls(response: Dict, ssh: bool) -> Dict:
    ...
    if response.get("repo"):
        clone_urls = {
            repo.get("name"): repo.get(repo_clone_url) for repo in response.get("repo")
        }
    if response.get("gist"):
        clone_urls.update(
            {gist.get("id"): gist.get("git_pull_url") for gist in response.get("gist")}
        )
    return clone_urls
...
def execute_cloning(url_list: dict):
    ...
    with Progress(
        bar_column, progress_console, time_elapsed_column, expand=True
    ) as progress:
