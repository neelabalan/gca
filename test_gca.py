...
from gca import construct_url, fetch_response, get_clone_urls, get_user_response, execute_cloning
...
class TestGca(unittest.TestCase):
    ...
    def test_fetch_response(self, mock_requests):
        mock_requests.get.side_effect = [
            MockResponse(
                json_data=[{'clone_url': 'https://gist.github.com/123456789.git', 'name': 'testrepo'}],
                status_code=200
            ),
            MockResponse(
                json_data=[],
        ...
        self.assertEqual(
            [{'clone_url': 'https://gist.github.com/123456789.git', 'name': 'testrepo'}],
            fetch_response('https://api.github.com/orgs/testuser/repos?per_page=100&page={page}') 
        )
    ...
    def test_get_user_response(self, mock_requests):
        mock_requests.get.return_value = MockResponse(json_data={'user': 'rick'}, status_code=404)
        self.assertEqual(
            get_user_response('rick'),
            {'user': 'rick'}
        )
